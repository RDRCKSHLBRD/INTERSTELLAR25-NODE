...
class PlaylistManager {
    ...
    try {
      ...
      item.querySelectorAll('.playlists-song').forEach(songEl => {
        songEl.addEventListener('click', (e) => {
          ...
          const albumId = songEl.dataset.albumId;
  ...
  async _playPlaylist(playlistId) {
    try {
      ...
      if (window.audioPlayer) {
        // songId is varchar, albumId may be int — audioPlayer handles coercion
        window.audioPlayer.playSong(first.song_id, first.album_id);
