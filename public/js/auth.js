...
class AuthManager {
  ...
  setupEventListeners() {
    ...
    if (loginBtn) {// auth.js - Authentication Handler with Cart Integration
  ...
  async handleLogin(e) {
    ...
    try {
      ...
      if (data.success) {
        ...
        // 🔄 REFRESH CART AFTER LOGIN - This is the key addition!
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (window.cartManager) {
          ...
          await window.cartManager.refreshCartAfterLogin(); // This handles both login/logout
