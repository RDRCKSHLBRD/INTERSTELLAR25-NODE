-- INTERSTELLAR DATABASE - COMPLETE SQLITE MIGRATION (V2)
-- Generated: 2026-02-08
-- Target: SQLite 3
-- Notes (V2):
-- - Drops legacy playlist_songs (old junction table) in cleanup
-- - Adds integrity CHECK for purchases (guest vs user)
-- - Restores key indexes used by the original Postgres migrations
-- - Adds cart_items uniqueness (cart_id, product_id)
-- - Adds updated_at triggers for all tables that have updated_at
-- - Seeds full catalogue: 1 artist, 25 albums, 232 songs
-- - Auto-generates products for albums + songs, then inserts physical cassette product
...
DROP TABLE IF EXISTS playlist_songs; -- legacy (Postgres-era) join table
...
CREATE TABLE purchases (
    ...
    updated_at DATETIME DEFAULT CURRENT_TIMESTAMP,
    CHECK (
        (purchase_type = 'user'  AND user_id IS NOT NULL)
        OR
        (purchase_type = 'guest' AND email   IS NOT NULL)
    )
);
...
CREATE UNIQUE INDEX IF NOT EXISTS idx_songs_album_track_unique ON songs(album_id, track_id);
...
CREATE INDEX IF NOT EXISTS idx_products_song_id ON products(song_id);
...
-- Sessions / Guest downloads
CREATE INDEX IF NOT EXISTS idx_user_sessions_user_id ON user_sessions(user_id);
CREATE INDEX IF NOT EXISTS idx_user_sessions_token ON user_sessions(session_token);
CREATE INDEX IF NOT EXISTS idx_guest_downloads_purchase_id ON guest_downloads(purchase_id);
...
BEGIN
  UPDATE purchases SET updated_at = CURRENT_TIMESTAMP WHERE id = OLD.id;
END;

CREATE TRIGGER IF NOT EXISTS update_guest_downloads_timestamp
AFTER UPDATE ON guest_downloads
BEGIN
  UPDATE guest_downloads SET updated_at = CURRENT_TIMESTAMP WHERE id = OLD.id;
END;

-- ==========================================
-- 5. DATA SEEDING (ARTISTS / ALBUMS / SONGS)
-- ==========================================
...
('2504', 'No Coincidence', '/api/audio/Signals/No-Coincidence.mp3', '05:44', 1, 25, 4),
...
-- ==========================================
-- 6. AUTO-GENERATE PRODUCTS (ALBUMS & SONGS)
-- ==========================================

-- Create products for Albums (digital)
...
-- Create products for Songs (digital) (Preventing duplicates)
INSERT INTO products (cat_id, price, name, catalogue_id, description, song_id, active)
SELECT 
    s.id,
    1.29,
    s.name,
...
-- Physical product (from legacy products seed): Natura (Cassette)
INSERT INTO products (cat_id, price, name, catalogue_id, description, active)
VALUES ('01020105', 25.00, 'Natura (Cassette)', '01040105', 'Physical cassette tape of Natura album', 1);

-- ==========================================
-- 7. VERIFICATION
-- ==========================================
SELECT 'Setup Complete! (V2)' as Status;
